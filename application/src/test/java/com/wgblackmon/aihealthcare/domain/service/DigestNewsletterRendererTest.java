...
class DigestNewsletterRendererTest {
    ...
    void buildDigest_bodyPreviewEndsAtSentenceBoundary() {
        String longBody = "First sentence about AI in healthcare and its transformative potential for patient outcomes. "
                + "Second sentence covers diagnostics and imaging breakthroughs using deep learning convolutional neural networks. "
                + "Third sentence discusses regulatory frameworks and compliance requirements across multiple international jurisdictions. "
                + "Fourth sentence about machine learning models and their applications in clinical settings and remote patient monitoring. "
                + "Fifth sentence about advanced natural language processing systems for electronic health records and documentation workflows. "
                + "Sixth sentence that goes well beyond the five hundred character limit and should definitely be cut off by the truncation logic.";
        when(articleIngestionPort.fetchRecentArticles(eq(1)))
        ...
        assertThat(result).isPresent();
        String html = result.get().htmlContent();
        assertThat(html).contains("First sentence about AI in healthcare and its transformative potential");
        assertThat(html).doesNotContain("...");
        assertThat(html).doesNotContain("Sixth sentence");
    }
    ...
    // -------------------------------------------------------------------------
    // Source name in meta line
    // -------------------------------------------------------------------------

    @Test
    void buildDigest_metaIncludesSourceName() {
        NewsArticle article = new NewsArticle(
                "a1", "Named Source Article", URI.create("https://example.com/named"),
                "Body text for preview", "AI Healthcare", null, null,
    ...
    @Test
    void buildDigest_noCitationBadgesOrSourcesSection() {
        when(articleIngestionPort.fetchRecentArticles(eq(1)))
                .thenReturn(List.of(
        ...
        assertThat(result).isPresent();
        String html = result.get().htmlContent();
        assertThat(html).doesNotContain("#source-1");
        assertThat(html).doesNotContain("id=\"source-1\"");
        assertThat(html).doesNotContain("=== SOURCES ===");
    }
